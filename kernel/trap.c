...
usertrap(void)
{
  ...
  if(which_dev == 2) {
    if (p->alarm_interval > 0) {
      p->alarm_interval_passed += 1;
      if (!p->in_alarm && p->alarm_interval_passed == p->alarm_interval - 1) {
        // avoid enter alarm again when alarm in process
        p->in_alarm = 1;

        // save registers of user process before enter handler
        ...
        // resume to handler when return to user space
        p->trapframe->epc = p->alarm_handler;

        p->alarm_interval_passed = 0;
      }
    }
    yield();
  }
